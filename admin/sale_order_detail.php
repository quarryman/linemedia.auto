...
$bUserCanViewOrder = (CUser::IsAdmin() || $access);
$bUserCanEditOrder = (CUser::IsAdmin() || $access);
$bUserCanCancelOrder = (CUser::IsAdmin() || $access);
$bUserCanPayOrder = (CUser::IsAdmin() || $access);
$bUserCanDeliverOrder = (CUser::IsAdmin() || $access);
$bUserCanDeleteOrder = (CUser::IsAdmin() || $access);
...
if (
    ...
    if (!$customTabber->Check()) {
        ...
        if ($action == "change_allow_payment" && $_SERVER["REQUEST_METHOD"] == "GET" ) {
            $lmorder = new LinemediaAutoOrder($ID);
            
            $sLM_ALLOW_PAYMENT = (isset($_GET['LM_ALLOW_PAYMENT']) && trim($_GET['LM_ALLOW_PAYMENT']) === 'Y') ? 'Y' : 'N';
            
            if (!$lmorder->setAllowPayemnt($ID, $sLM_ALLOW_PAYMENT)) {
                if ($ex = $APPLICATION->GetException()) {
                    $errorMessage .= $ex->GetString();
                } else {
                    $errorMessage .= GetMessage('ERROR_CHANGE_ALLOW_PAYEMNT').'. ';
                }
            }
            if (strlen($errorMessage) <= 0) {
                LocalRedirect("linemedia.auto_sale_order_detail.php?ID=".$ID."&result=ok_allow_payment&lang=".LANG.GetFilterParams("filter_", false));
            }
        } elseif (
            ...
            if (strlen($errorMessage) <= 0)
                LocalRedirect("linemedia.auto_sale_order_detail.php?ID=".$ID."&result=ok_status&lang=".LANG.GetFilterParams("filter_", false));
        }
            ...
            && $_SERVER["REQUEST_METHOD"] == "GET")
        {
            ...
            if (strlen($errorMessage) <= 0)
                LocalRedirect("linemedia.auto_sale_order_detail.php?ID=".$ID."&result=ok_cancel&lang=".LANG.GetFilterParams("filter_", false));
        }
            ...
            && $_SERVER["REQUEST_METHOD"] == "GET")
        {
            ...
            if (strlen($errorMessage) <= 0)
                LocalRedirect("linemedia.auto_sale_order_detail.php?ID=".$ID."&result=ok_pay&lang=".LANG.GetFilterParams("filter_", false));
        }
            ...
            && $_SERVER["REQUEST_METHOD"] == "GET")
        {
            ...
            if (strlen($errorMessage) <= 0)
                LocalRedirect("linemedia.auto_sale_order_detail.php?ID=".$ID."&result=ok_delivery&lang=".LANG.GetFilterParams("filter_", false));
        }
            ...
            && $_SERVER["REQUEST_METHOD"] == "POST")
        {
            ...
            if (strlen($errorMessage) <= 0)
                LocalRedirect("linemedia.auto_sale_order_detail.php?ID=".$ID."&result=ok_comment&lang=".LANG.GetFilterParams("filter_", false));
        }
            ...
            && $_SERVER["REQUEST_METHOD"] == "GET")
        {
            ...
            if (strlen($errorMessage) <= 0) {
                if ($arOrder["PS_STATUS"] == "Y" && $arOrder["PAYED"] == "N") {
                    if (
                        $arOrder["CURRENCY"] == $arOrder["PS_CURRENCY"]
            ...
            if (strlen($errorMessage) <= 0)
                LocalRedirect("linemedia.auto_sale_order_detail.php?ID=".$ID."&result=ok_ps&lang=".LANG.GetFilterParams("filter_", false));
        }
    }
...
?>
    <script>
    function ChangeSupplier(param){
        var s_url = '';
        s_url += '&TakeBasketID=' + param.ChangeBasketID;
        s_url += '&ChangeProdNewID=' + param.id;
        jQuery.each(param, function(i, val){
            s_url += '&ChangeProdNew[' + i + ']=' +  val;
        });
        
        location.replace('<?=$APPLICATION->GetCurPageParam('Action=ChangeSupplier', Array('TakeBasketID', 'ChangeProdNewID', 'Action'));?>' + s_url);
        return true;
    }
    </script>
<?
//статусы на изменение поставщика
$aOptionChangeStatusSuppAccList = explode(',', COption::GetOptionString('linemedia_auto_portal', 'LM_AUTO_STATUS_CHANGE_SUPPLIER_ACCESS_LIST'));
...
$aMenu[] = array(
        ...
        "LINK" => "/bitrix/admin/linemedia.auto_sale_order_detail.php?ID=".$ID."&export=excel&lang=".LANGUAGE_ID.GetFilterParams("filter_"),
        "ICON"=>"btn_excel",
    );
*/
...
if ($saleModulePermissions == "W" || $arOrder["PAYED"] != "Y" && $bUserCanDeleteOrder) {
    $aMenu[] = array(
            ...
            "LINK" => "javascript: if(confirm('".GetMessage("SODN_CONFIRM_DEL_MESSAGE")."')) window.location='linemedia.auto_sale_orders_list.php?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get().GetFilterParams("filter_")."'",
            "WARNING" => "Y",
            "ICON" => "btn_delete",
        );
}
...
if (strlen($result) > 0) {
    ...
    elseif ($result == "ok_allow_payment")
        $okMessage = GetMessage("SOD_OK_ALLOW_PAYEMNT");
    ...
    elseif ($result == "ok_chng_supp")
        $okMessage = 'Поставщик успешно изменен';
        ...
        && is_file($_SERVER["DOCUMENT_ROOT"].$customOrderView))
    {
        ...
        $tabControl->BeginNextFormTab();
            ...
            $tabControl->AddSection("order_id", GetMessage("P_ORDER_ID"));
                ...
                $tabControl->BeginCustomField("ORDER_STATUS", GetMessage("SOD_CUR_STATUS"));
                    ...
                    <tr>
                        ...
                        <td width="60%">
                        <form></form>
                            <form method="GET" action="/bitrix/admin/linemedia.auto_sale_order_detail.php" name="change_status">
                ...
                $tabControl->BeginCustomField("ORDER_CANCELED", GetMessage("P_ORDER_CANCELED"));
                    ...
                    <tr>
                        <td width="40%"><?echo $tabControl->GetCustomLabelHTML()?></td>
                        <td width="60%">
                            <form method="get" action="/bitrix/admin/linemedia.auto_sale_order_detail.php" name="change_cancel">
                                <script language="JavaScript">
                                <!--
            ...
            foreach ($arOrderProperties as $arOrderProps) {
                ...
                $aOrder['PROPS'][$arOrderProps['CODE']] = $arOrderProps;
                
                if ($arOrderProps['CODE']  === 'ALLOW_PAYMENT') {
                    continue;
                }
                ...
                if ($iGroup != IntVal($arOrderProps["PROPS_GROUP_ID"]))
                {
                    ...
                    <tr>
                        ...
                        {
                            if($arOrderProps['CODE'] == 'LM_MANAGER'){
                                $aPropUser = CUser::GetByID($arOrderProps['VALUE'])->Fetch();
                                echo '[' . $aPropUser['ID'] . '] ' . $aPropUser['LAST_NAME'] . ' ' . $aPropUser['NAME'];
                                unset($aPropUser);
                            }elseif($arOrderProps['CODE'] == 'LM_BRANCH'){
                                if(!CModule::IncludeModule("iblock")){
                                    ShowError('IBLOCK MODULE NOT INSTALLED');
                                    return;
                                }

                                $aOrderBranchs = CIBlockElement::GetByID($arOrderProps["VALUE"])->Fetch();
                                if(!empty($aOrderBranchs['NAME'])){
                                    echo htmlspecialcharsEx($aOrderBranchs['NAME']);
                                }
            ...
            if (strlen($arOrder["USER_DESCRIPTION"]) > 0)
            {
                $tabControl->BeginCustomField("ORDER_USER_COMMENT", GetMessage("P_ORDER_USER_COMMENT"));
                    ...
                    <tr>
                        ...
                        <td width="60%"><?= $arOrder["USER_DESCRIPTION"] ?></td>
                    </tr>
                    <?
            ...
            $tabControl->AddSection("order_payment", GetMessage("P_ORDER_PAYMENT"));
                ...
                $tabControl->BeginCustomField("ALLOW_PAYMENT", GetMessage('P_ALLOW_PAYMENT'));
                if (isset($aOrder['PROPS']['ALLOW_PAYMENT']['VALUE']) && $aOrder['PROPS']['ALLOW_PAYMENT']['VALUE'] == 'Y'){
                    $sLM_ALLOW_PAYMENT = 'Y';
                } else {
                    $sLM_ALLOW_PAYMENT = 'N';
                }
                ?>
                    <tr>
                        <td width="40%"><?= $tabControl->GetCustomLabelHTML() ?></td>
                        <td width="60%">
                            <form method="get" action="/bitrix/admin/linemedia.auto_sale_order_detail.php" name="change_allow_payment">
                                <? if ($sLM_ALLOW_PAYMENT == 'Y') { ?>
                                    <?= GetMessage('YES') ?>
                                <? } else { ?>
                                    <?= GetMessage('NO') ?>
                                <? } ?>
                                <?= GetFilterHiddens("filter_") ?>
                                <?= bitrix_sessid_post() ?>
                                <input type="hidden" name="LM_ALLOW_PAYMENT" value="<?= (($sLM_ALLOW_PAYMENT == "Y") ? "N" : "Y"); ?>" />
                                <input type="hidden" name="ID" value="<?= $ID ?>" />
                                <input type="hidden" name="action" value="change_allow_payment" />
                                <input type="hidden" name="lang" value="<?= LANG ?>" />
                                <? if ($sLM_ALLOW_PAYMENT == 'Y') { ?>
                                    <input type="submit" value="<?= GetMessage('DISALLOW') ?>" />
                                <? } else { ?>
                                    <input type="submit" value="<?= GetMessage('ALLOW') ?>" />
                                <? } ?>
                            </form>
                        </td>
                    </tr>
                ...
                $tabControl->EndCustomField("ALLOW_PAYMENT", '');
                unset($sLM_ALLOW_PAYMENT);
                ...
                $tabControl->BeginCustomField("ORDER_PAYED", GetMessage("P_ORDER_PAYED"));
                    ...
                    <tr>
                        <td width="40%"><?echo $tabControl->GetCustomLabelHTML()?></td>
                        <td width="60%">
                            <form method="get" action="/bitrix/admin/linemedia.auto_sale_order_detail.php" name="change_pay_form">
                                <script language="JavaScript">
                                <!--
                    ...
                    <tr>
                        ...
                        <td width="60%">
                            ...
                            if ($arPaySys["PSA_HAVE_RESULT"] == "Y" || strlen($arPaySys["PSA_RESULT_FILE"]) > 0)
                            {
                                ?>
                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                <a href="linemedia.auto_sale_order_detail.php?ID=<?= $ID ?>&action=ps_update&lang=<?= LANG ?><?echo GetFilterParams("filter_")?>&<?= bitrix_sessid_get() ?>"><?echo GetMessage("P_ORDER_PS_STATUS_UPDATE") ?> &gt;&gt;</a>
                                <?
                            }
                            ?>
                        </td>
                ...
                elseif ($arPaySys["PSA_HAVE_RESULT"] == "Y" || strlen($arPaySys["PSA_RESULT_FILE"]) > 0)
                {
                    ...
                    <tr>
                        <td width="40%"><?echo $tabControl->GetCustomLabelHTML()?></td>
                        <td width="60%"><a href="linemedia.auto_sale_order_detail.php?ID=<?= $ID ?>&action=ps_update&lang=<?= LANG ?><?= GetFilterParams("filter_") ?>&<?= bitrix_sessid_get() ?>"><?= GetMessage("P_ORDER_PS_STATUS_UPDATE") ?> &gt;&gt;</a></td>
                    </tr>
                    <?
                    $tabControl->EndCustomField("ORDER_PS_STATUS", '');
                }
            $tabControl->AddSection("order_delivery", GetMessage("P_ORDER_DELIVERY"));
                ...
                $tabControl->BeginCustomField("ORDER_ALLOW_DELIVERY", GetMessage("P_ORDER_ALLOW_DELIVERY"));
                    ...
                    <tr>
                        <td width="40%"><?echo $tabControl->GetCustomLabelHTML()?></td>
                        <td width="60%">
                            <form method="get" action="/bitrix/admin/linemedia.auto_sale_order_detail.php" name="change_delivery_form">
                            <script language="JavaScript">
                            <!--
                ...
                <tr>
                    <td width="40%"><?echo $tabControl->GetCustomLabelHTML()?></td>
                    <td width="60%">
                        <form method="post" action="/bitrix/admin/linemedia.auto_sale_order_detail.php" name="change_comments">
                            <script language="JavaScript">
                            <!--
                ...
                <tr>
                    <td colspan="2">
                        ...
                        $dbBasket = CSaleBasket::GetList(
                                ...
                                false,
                                false,
                                array("ID", "DETAIL_PAGE_URL", "NAME", "NOTES", "QUANTITY", "PRICE", "CURRENCY", 'PRODUCT_ID', "PRODUCT_XML_ID", "DISCOUNT_NAME", "DISCOUNT_VALUE", "DISCOUNT_COUPON", "WEIGHT", 'MODULE')
                            );
                        ?>
                        <table cellpadding="3" cellspacing="1" border="0" width="100%" class="internal">
                            <tr class="heading">
                                <td><?= GetMessage("SALE_F_NAME")?></td>
                                <td><?= GetMessage("SALE_F_PROPS")?></td>
                                <td><?= GetMessage("SALE_F_BASKET_DISCOUNT")?></td>
                                <? /* <td><?= GetMessage("SALE_F_PTYPE")?></td> */ ?>
                                <td><?= GetMessage("SALE_F_NOTES")?></td>
                                <td><?= GetMessage("SALE_F_WEIGHT")?></td>
                            ...
                            while ($arBasket = $dbBasket->Fetch()) {
                                ...
                                while ($arBasketProps = $dbBasketProps->Fetch()){
                                    $aItemProps[$arBasketProps['CODE']] = $arBasketProps;
                                }
                                unset($arBasketProps);
                                ?>
                                <tr>
                                    <td valign="top">
                                        ...
                                        ID: <?= $arBasket['ID'] ?>
<?
//если позволено меня поставщика для товара, выводим ссылку
if(
   isset($aItemProps['STATUS']['VALUE']) &&
   is_array($aOptionChangeStatusSuppAccList) &&
   count($aOptionChangeStatusSuppAccList) > 0 &&
   in_array($aItemProps['STATUS']['VALUE'], $aOptionChangeStatusSuppAccList) &&
   $arBasket['MODULE'] === 'linemedia_auto_portal'
   ):
    /*
        Передаем данные первоначальной позиции в pop-up
    */
    $s_link_uri_base_price_product = '';
    if(isset($aItemProps['supplier_api']['VALUE'])){
        $s_link_uri_base_price_product = '&ShowBasePriceForProduct[id]=' . $arBasket['PRODUCT_ID'] . '_' . $aItemProps['supplier_api']['VALUE'];

        if(isset($aItemProps['supplier_api']['VALUE'])){
        $s_link_uri_base_price_product .= '&ShowBasePriceForProduct[api_supplier]=' . $aItemProps['supplier_api']['VALUE'];
        }

        if(isset($aItemProps['art']['VALUE'])){
        $s_link_uri_base_price_product .= '&ShowBasePriceForProduct[art]=' . $aItemProps['art']['VALUE'];
        }

        if(isset($aItemProps['price_id']['VALUE'])){
        $s_link_uri_base_price_product .= '&ShowBasePriceForProduct[priceId]=' . $aItemProps['price_id']['VALUE'];
        }

        if(isset($aItemProps['brand_title']['VALUE'])){
        $s_link_uri_base_price_product .= '&ShowBasePriceForProduct[brand_title]=' . $aItemProps['brand_title']['VALUE'];
        }
    }else{
        $s_link_uri_base_price_product = '&ShowBasePriceForProduct[id]=' . $arBasket['PRODUCT_ID'];
    }
?>
                                        <br /><br />
<?
...
?>
                                    ...
                                    <td valign="top">
                                        <?
                                        foreach($aItemProps AS $arBasketProps){
                                            switch($arBasketProps['CODE']){
                                                case 'STATUS':
                                                    echo "<i>".htmlspecialcharsEx($arBasketProps["NAME"]).":</i> ";
                                                    $aStatus = CSaleStatus::GetByID($arBasketProps["VALUE"]);
                                                    if($aStatus !== false){
                                                        echo htmlspecialcharsEx($aStatus["NAME"]);
                                                    }else{
                                                ...
                                                case 'EMP_STATUS_ID':
                                                    echo "<i>".htmlspecialcharsEx($arBasketProps["NAME"]).":</i> ";
                                                    $aUser = CUser::GetByID($arBasketProps["VALUE"])->Fetch();
                                                    if($aUser !== false){
                                                        echo "[<a href=\"/bitrix/admin/user_edit.php?ID=" . $arBasketProps["VALUE"] . "&lang=".LANG."\">" . $arBasketProps["VALUE"] . "</a>] ";
                                                        echo htmlspecialcharsEx($aUser["NAME"].((strlen($aUser["NAME"])<=0 || strlen($aUser["LAST_NAME"])<=0) ? "" : " ").$aUser["LAST_NAME"]." (".$aUser["LOGIN"].")");
                                                    }else{
                                                        echo htmlspecialcharsEx($arBasketProps["VALUE"]);
                                                    }
                                                    unset($aUser);
                                                ...
                                                case 'supplier_id':
                                                    echo "<i>".htmlspecialcharsEx($arBasketProps["NAME"]).":</i> ";
                                                    $sLM_AUTO_SUPPLIERS_IBLOCK_ID = COption::GetOptionString('linemedia_auto_portal', 'LM_AUTO_SUPPLIERS_IBLOCK_ID', '0');
                                                    $aSupplier = CIBlockElement::GetList(Array(), Array('PROPERTY_id' => $arBasketProps["VALUE"], 'IBLOCK_ID' => $sLM_AUTO_SUPPLIERS_IBLOCK_ID), false, false, Array('ID', 'IBLOCK_TYPE_ID', 'IBLOCK_ID', 'NAME'))->Fetch();
                                                    if($aSupplier !== false){
                                                        echo  "[<a href=\"/bitrix/admin/iblock_element_edit.php?ID=" . $aSupplier["ID"] . "&type=" . $aSupplier['IBLOCK_TYPE_ID'] . "&lang=".LANG."&IBLOCK_ID=".$aSupplier["IBLOCK_ID"]."\">".$aSupplier["ID"]."</a>] ";
                                                        echo htmlspecialcharsEx($aSupplier['NAME']);
                                                    }else{
                                                        echo htmlspecialcharsEx($arBasketProps["VALUE"]);
                                                    }
                                                    unset($aSupplier);
                                                ...
                                                case 'base_price':
                                                    echo "<i>".htmlspecialcharsEx($arBasketProps["NAME"]).":</i> ";
                                                    echo htmlspecialcharsEx( CurrencyFormat($arBasketProps["VALUE"], $arOrder["CURRENCY"]) );
                                                    echo "<br>";
                                                break;
                                                default:
                                                    echo "<i>".htmlspecialcharsEx($arBasketProps["NAME"]).":</i> ";
                                    ...
                                    <td valign="top">
                                        <?= htmlspecialcharsEx($arBasket['NOTES']) ?>
                                    </td>
                                    <td valign="top">
                                        <?= $arBasket["WEIGHT"] ?> <?=$WEIGHT_UNIT?>
                                    </td>
                                    <td valign="top">
                                        <?= $arBasket["QUANTITY"] ?>
                                    </td>
                                    <td align="right" valign="top">
                                        <?= SaleFormatCurrency($arBasket["PRICE"], $arBasket["CURRENCY"]) ?>
                                    </td>
                                </tr>
                                <?
                            }
