...
while ($arEvent = $events->Fetch()) {
    $access = (bool) ExecuteModuleEventEx($arEvent, array($USER->GetID(), $ID));
    if (!$access) {
        break;
    }
}
...
$bUserCanViewOrder = ($USER->IsAdmin() || $access);
$bUserCanEditOrder = ($USER->IsAdmin() || $access);
$bUserCanCancelOrder = ($USER->IsAdmin() || $access);
$bUserCanPayOrder = ($USER->IsAdmin() || $access);
$bUserCanDeliverOrder = ($USER->IsAdmin() || $access);
$bUserCanDeleteOrder = ($USER->IsAdmin() || $access);
...
/***** /DELIVERY *****/
    ...
    if (strlen($errorMessage) <= 0)
    {
        ...
        if (strlen($apply) <= 0)
            LocalRedirect("/bitrix/admin/linemedia.auto_sale_orders_list.php?lang=".LANG.GetFilterParams("filter_", false));
    }
    else
    {
...
else
{
    ...
    $aMenu = array(
            array(
                ...
                "LINK" => "/bitrix/admin/linemedia.auto_sale_order_edit.php?ID=".$ID."&dontsave=Y&lang=".LANGUAGE_ID.GetFilterParams("filter_")
            )
        );
    ...
    if ($saleModulePermissions == "W" || $str_PAYED != "Y" && $bUserCanDeleteOrder)
    {
        $aMenu[] = array(
                ...
                "LINK" => "javascript:if(confirm('".GetMessage("SOEN_CONFIRM_DEL_MESSAGE")."')) window.location='linemedia.auto_sale_orders_list.php?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get().urlencode(GetFilterParams("filter_"))."'",
                "WARNING" => "Y"
            );
    }
        ...
        ?>
            ...
            else
            {
                ...
                while ($arBasket = $dbBasket->GetNext()) {
                    ...
                    $arBasket['CAN_ADMIN_EDIT'] = true;
                    $arBasket['CAN_ADMIN_DELETE'] = true;
            ...
            /*
             * Создаём событие "Отображение корзин при изменении заказа заказа"
             */
            $events = GetModuleEvents("linemedia.auto", "OnOrderEditShowBasketList");
            while ($arEvent = $events->Fetch()) {
                ExecuteModuleEventEx($arEvent, array(&$arBasketItem));
            }
            ...
            foreach ($arBasketItem as $val) {
                ...
                <tr id="BASKET_TABLE_ROW_<?= $val["ID"] ?>">
                    ...
                    <td>
                        <? if ($val['CAN_ADMIN_EDIT']) { ?>
                            <a href="javascript: ShowProductEdit(<?= $val["ID"] ?>);"><?= GetMessage("SOE_JS_EDIT") ?></a>
                        <? } ?>
                        <br/>
                        <? if ($val['CAN_ADMIN_DELETE']) { ?>
                            <a href="javascript: DeleteProduct(<?= $val["ID"] ?>);"><?= GetMessage("SOE_JS_DEL") ?></a>
                        <? } ?>
                        <? if (!$val['CAN_ADMIN_EDIT'] && !$val['CAN_ADMIN_DELETE']) { ?>
                            <?= GetMessage('SOE_JS_CANT_EDIT') ?>
                        <? } ?>
                    </td>
                </tr>
