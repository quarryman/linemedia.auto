...
if($page == 'part_details') {
	...
	try {
		$data = $api->query('getOriginalChevroletArticleDetails', array('article_id' => $arVariables['ARTICLE_ID']));
	} catch (Exception $e) {
    ...
    */
	...
	try {
		$data = $api->query('getOriginalChevroletArticles', array('group_section_id' => $arVariables['GROUP_SECTION']));
	} catch (Exception $e) {
		$arResult['ERROR'] = $e->GetMessage(); 
		$this->IncludeComponentTemplate('error');
		return;
	...
	*/
	...
	try {
		$data = $api->query('getOriginalChevroletGroupSections', array('group_id' => $arVariables['GROUP']));
	} catch (Exception $e) {
		$arResult['ERROR'] = $e->GetMessage(); 
		$this->IncludeComponentTemplate('error');
		return;
	...
	try {
		$data = $api->query('getOriginalChevroletGroups', array('group_type_id' => $arVariables['GROUP_TYPE']));
	} catch (Exception $e) {
		$arResult['ERROR'] = $e->GetMessage(); 
		$this->IncludeComponentTemplate('error');
		return;
	...
	try {
		$data = $api->query('getOriginalChevroletModifications', array('model_id' => $arVariables['MODEL']));
	} catch (Exception $e) {
		$arResult['ERROR'] = $e->GetMessage(); 
		$this->IncludeComponentTemplate('error');
		return;
	}
	$arResult['MODIFICATIONS'] = $data['data']['modifications'];
	$arResult['MODEL'] = $data['data']['model'];
	...
	try {
		$data = $api->query('getOriginalChevroletModels', array('brand_id' => $arVariables['BRAND']));
	} catch (Exception $e) {
		$arResult['ERROR'] = $e->GetMessage(); 
		$this->IncludeComponentTemplate('error');
		return;
