...
$.widget('ui.spinner_control', {
	_init: function() {
		...
		var self = this;
		this.element
			.addClass('ui-spinner_control-box')
		...
		.wrap('<div>')
		.parent()
			.addClass('ui-spinner_control')
			.append('<button class="ui-spinner_control-up" type="button">&#9650;</button>')
			.find('.ui-spinner_control-up')
				.bind('mousedown', function(e) {
					$(this).addClass('ui-spinner_control-pressed');
					if (!self.counter) {
						self.counter = 1;
					}
				...
				.bind('mouseup', function(e) {
					$(this).removeClass('ui-spinner_control-pressed');
					if (self.counter == 1) {
						self._up(e);
					}
					self._mouseup(e);
				...
				.bind('keydown.spinner_control', function(e) {
					var KEYS = $.keyCode;
					if (e.keyCode == KEYS.SPACE || e.keyCode == KEYS.ENTER) {
						...
						self.element.siblings('.ui-spinner_control-down').focus();
					} else if (e.keyCode == KEYS.LEFT) {
						self.element.focus();
					}
				})
				.bind('keyup.spinner_control', function(e) {
					$(this).removeClass('ui-spinner_control-pressed');
					self.counter = 0;
					self._propagate('change', e);
				})
			.end()
			.append('<button class="ui-spinner_control-down" type="button">&#9660;</button>')
			.find('.ui-spinner_control-down')
				.bind('mousedown', function(e) {
					$(this).addClass('ui-spinner_control-pressed');
					if (!self.counter) {
						self.counter = 1;
				...
				.bind('keydown.spinner_control', function(e) {
					...
					if (e.keyCode == KEYS.SPACE || e.keyCode == KEYS.ENTER) {
						...
						self.element.siblings('.ui-spinner_control-up').focus();
					}
				})
				.bind('keyup.spinner_control', function(e) {
					$(this).removeClass('ui-spinner_control-pressed');
		...
		// Manipulate height of spinner_control.
		this._items = this.element.children().length;
		if (this._items > 1) {
			var height = this.element.outerHeight()/this._items;
			this.element
			.addClass('ui-spinner_control-list')
			.height(height)
			.children()
				.addClass('ui-spinner_control-listitem')
				.height(height)
				.css('overflow', 'hidden')
			.end()
			.parent()
		...
		.bind('blur.spinner_control', function(e) {
			self._cleanUp();
		});
	...
	_setValue: function(newVal) {
		...
		this.element.val(
			this.options.currency ? 
				$.ui.spinner_control.format.currency(newVal, this.options.currency) : 
				$.ui.spinner_control.format.number(newVal, this._decimals)
		);
	},
	_animate: function(d) {
		if (this.element.hasClass('ui-spinner_control-list') && ((d == 'up' && this._getValue() <= this.options.max) || (d == 'down' && this._getValue() >= this.options.min)) ) {
	...
	_addItem: function(obj, fmt) {
		if (!this.element.is('input')) {
			...
			this.element.append('<'+ wrapper +' class="ui-spinner_control-dyn">'+ html + '</'+ wrapper +'>');
		}
	},
	...
	destroy: function() {
		if (!$.data(this.element[0], 'spinner_control')) {
			return;
		}
		if ($.fn.mousewheel) {
		...
		this.element
			.removeClass('ui-spinner_control-box ui-spinner_control-list')
			.removeAttr('disabled')
			.removeAttr('autocomplete')
			.removeData('spinner_control')
			.unbind('.spinner_control')
			.siblings()
				.remove()
			.end()
			.children()
				.removeClass('ui-spinner_control-listitem')
				.remove('.ui-spinner_control-dyn')
			.end()
			.parent()
				.removeClass('ui-spinner_control ui-spinner_control-disabled')
	...
	enable: function() {
		this.element
			...
			.siblings()
				.removeAttr('disabled')
			.parent()
				.removeClass('ui-spinner_control-disabled');
		this.disabled = false;
	},
	disable: function() {
		this.element
			...
			.siblings()
				.attr('disabled', true)
			.parent()
				.addClass('ui-spinner_control-disabled');
		this.disabled = true;
	}
});

$.extend($.ui.spinner_control, {
	defaults: {
		decimals: 0,
		stepping: 1,
		start: 0,
