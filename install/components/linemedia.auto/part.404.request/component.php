...
if (CModule::IncludeModule("form"))
{
    ...
    if (!($fid = check404FormExistence())) {
        create404Form();
        $fid = check404FormExistence();
    }

    $arParams['WEB_FORM_ID'] = $fid;
				...
				if (check_bitrix_sessid())
				{
					...
					if($RESULT_ID = CFormResult::Add($arParams["WEB_FORM_ID"], $arResult["arrVALUES"]))
					{
						...
						if ($arResult["F_RIGHT"] >= 15)
						{
							...
							{
                                $arResult['MESSAGE'] = GetMessage('FORM_OK');
					...
					switch ($arAnswer["FIELD_TYPE"])
					{
						...
						case "text":
                            ...
                            switch ($FIELD_SID) {
                                ...
                                case 'EMAIL':
                                    if ($GLOBALS['USER']->IsAuthorized())
                                        $value = $GLOBALS['USER']->GetEmail();
                                    break;
                                case 'WHAT_FIND':
                                        $value = trim($arParams['BRAND_TITLE'].' '.$arParams['WHAT_FIND']);
                                    break;
                            }
							$inp = CForm::GetTextField(
								$arAnswer["ID"],
								$value,
