...
$arResult['AJAX_URL'] = $this->GetPath();
...
if (!isset($arParams['AJAX_RECALC'])) {
    $arParams['AJAX_RECALC'] = 'Y';
}
...
if (strlen($_REQUEST["BasketRemove"]) > 0 || strlen($_REQUEST["BasketRefresh"]) > 0 || strlen($_REQUEST["BasketOrder"]) > 0 || strlen($_REQUEST["action"]) > 0) {
	if (strlen($_REQUEST["action"]) > 0) {
        ...
        if (strlen($_REQUEST['BasketRemove']) > 0) {
            $basketIDs = array_map('intval', (array) $_REQUEST['deletes']);
	...
	if (strlen($_REQUEST['BasketOrder']) > 0) {
	    ...
	    unset($_REQUEST['BasketRemove']);
		unset($_REQUEST['BasketRefresh']);
		unset($_REQUEST['BasketOrder']);
		
		/*
		 * Если не редиректить - не работает пересчёт в корзине из-за наличия в GET delete.
		 */
