...
<? if (isset($arResult['BRANDS'])): ?>
    ...
    <table cellpadding="0" cellspacing="0" class="tecdoc">
    <tbody>
        <tr>
            <td>
            <?
            $i = 0;
            $letter = '';
            // если не определено -- будет один столбец.
            $arParams['COLUMNS_COUNT'] = max(intval($arParams['COLUMNS_COUNT']), 1);
            $letters_in_column = round(count($arResult['BRANDS_BY_LETTER_COUNT']) / $arParams['COLUMNS_COUNT']);
            // в последнем столбце может быть чуть больше значений,чем $letters_in_column,
            // поэтому сохраняем индекс столбца
            $columnIndex = 1;
            $ul_needs_close = 0; // флаг того, что надо закрыть <ul>. не думаю, что хитрое мат. условие будет понятней, поэтому так.
                        
            foreach ($arResult['BRANDS'] as $value):
            	
            	
            	if(!$bIsAdmin AND $value['hidden'] == 'Y')
            		continue;
            	
                if (($value['manuName']{0} != $letter) && (($bIsAdmin === true) || ($bIsAdmin === false && !isset($arResult['ACCESS_LIST_DISABLE'][$value['manuId']])))):
                    if ($letter != '') { ?>
                        </ul>
                    <?
                        $ul_needs_close = 0;
                    }
                    $letter = $value['manuName']{0};
                    // дошли до макс. допустимого кол-ва выведенных букв и столбец не последний
                    // (в  нём может быть как больше,так и меньше значений)
                    if ($i >= $letters_in_column && $columnIndex != $arParams['COLUMNS_COUNT']) {
                        $i = 0;
                        ++$columnIndex; ?>
                        </td><td>
                    <? }
                ...
                <? $manu = ($is_brand_car_name) ? strtolower($value['manuName']) : $value['manuId']; ?>
                <? if ($bIsAdmin) { ?>
                
                    <li>
                        <input type="checkbox" class="f_access" value="<?= $value['manuId'] ?>" name="BRANDS" <?=((!isset($arResult['ACCESS_LIST_DISABLE'][$value['manuId']]))?' checked="checked"':'');?> />
                        <? if (!empty($value['LOGO'])) { ?>
                            <img class="brand-logo" src="<?= $value['LOGO'] ?>" />
                        <? } ?>
                        <a class="m_select" href="<?= $arParams['SEF_FOLDER'] ?><?= $manu ?>/"><?= $value['manuName'] ?></a>
                    </li>
                <?  } elseif ($bIsAdmin === false && !isset($arResult['ACCESS_LIST_DISABLE'][$value['manuId']])) { ?>
                    <li>
                        <? if (!empty($value['LOGO'])) { ?>
                            <img class="brand-logo" src="<?= $value['LOGO'] ?>" />
                        <? } ?>
...
<? elseif (isset($arResult['MODEL_GROUPS'])): ?>
    ...
    <table cellpadding="0" cellspacing="0" class="tecdoc model_select silver_table">
        <thead>
            <tr>
...
<? elseif (isset($arResult['MODIFICATION'])): ?>
    ...
    <table cellpadding="0" cellspacing="0" class="model_select tecdoc silver_table">
        <thead>
            <tr>
                            ...
                            <table>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <b><?= $dop_info['vehicleDetails2']['constructionType'] ?></b>
                                        </td>
                                    </tr>
                                    <tr>
...
<? elseif (isset($arResult['DETAILS'])): ?>
    ...
    <table cellpadding="0" cellspacing="0" class="model_select tecdoc silver_table" id="tecdoc_search_result">
        <thead>
            <tr>
