...
if($page == 'part_details') {
	...
	try {
		$data = $api->query('getOriginalHyundaiArticleDetails', array('article_id' => $arVariables['ARTICLE_ID']));
	} catch (Exception $e) {
    ...
    */
	...
	try {
		$data = $api->query('getOriginalHyundaiArticles', array('group_section_id' => $arVariables['GROUP_SECTION']));
	} catch (Exception $e) {
		$arResult['ERROR'] = $e->GetMessage(); 
		$this->IncludeComponentTemplate('error');
		return;
	...
	*/
	...
	try {
		$data = $api->query('getOriginalHyundaiGroupSections', array('group_id' => $arVariables['GROUP']));
	} catch (Exception $e) {
		$arResult['ERROR'] = $e->GetMessage(); 
		$this->IncludeComponentTemplate('error');
		return;
	...
	try {
		$data = $api->query('getOriginalHyundaiGroups', array('group_type_id' => $arVariables['GROUP_TYPE']));
	} catch (Exception $e) {
		$arResult['ERROR'] = $e->GetMessage(); 
		$this->IncludeComponentTemplate('error');
		return;
	...
	try {
		$data = $api->query('getOriginalHyundaiModifications', array('model_id' => $arVariables['MODEL']));
	} catch (Exception $e) {
		$arResult['ERROR'] = $e->GetMessage(); 
		$this->IncludeComponentTemplate('error');
		return;
	...
	try {
		$data = $api->query('getOriginalHyundaiModels', array('brand_id' => $arVariables['BRAND']));
	} catch (Exception $e) {
		$arResult['ERROR'] = $e->GetMessage(); 
		$this->IncludeComponentTemplate('error');
		return;
