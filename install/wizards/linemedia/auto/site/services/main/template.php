...
$bitrixTemplateDir = $_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/templates/".$newTemplateName;
...
if ($arSite = $obSite->Fetch()) {
	...
	while ($arTemplate = $obTemplate->Fetch()) {
		if (!$found && strlen(trim($arTemplate["CONDITION"])) <= 0) {
			$arTemplate["TEMPLATE"] = $newTemplateName;
			$found = true;
		}
	...
	if (!$found) {
		$arTemplates []= array("CONDITION" => "", "SORT" => 150, "TEMPLATE" => $newTemplateName);
    }
...
CWizardUtil::ReplaceMacros($_SERVER['DOCUMENT_ROOT'].BX_PERSONAL_ROOT."/templates/".WIZARD_TEMPLATE_ID."/footer.php", array("WIZ_SITE_DIR" => WIZARD_SITE_DIR));
CWizardUtil::ReplaceMacros($_SERVER['DOCUMENT_ROOT'].BX_PERSONAL_ROOT."/templates/".WIZARD_TEMPLATE_ID."/header.php", array("WIZ_SITE_DIR" => WIZARD_SITE_DIR));
