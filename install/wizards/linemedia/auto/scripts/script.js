...
jQuery('#regFormLink').live('click', function(){
    $('#regForm').expose({
        loadSpeed: 0,
//        closeOnClick: false,
        color: '#000',
        ...
        onLoad: function(){
            $('#regForm').show();
            /*if (!$('#login-field').val().length)
                $('#login-field').focus();
            else
                $('#password-field').focus();
            $('#login-form').parent().addClass('ie7-auth-zindex');*/
        },
        onBeforeClose: function(){
            $('#regForm').hide();
            /*$('#login-form').parent().removeClass('ie7-auth-zindex');*/
        }
    });
    $('#regForm a.close').live('click', function(){
        $.mask.close();
    });
});
